...
function! <SID>GetSingleParamInfo(s, isparam)
	...
	if (g:filetype_php==1)
		if (match(single_param, '^\$bol\S\+$')!=-1)
			let type = 'bool'
		elseif (match(single_param, '^\$int\S\+$')!=-1)
			let type = 'int'
		elseif (match(single_param, '^\$str\S\+$')!=-1)
			let type = 'string'
		endif
	endif
	...
	if (a:isparam && strlen(type)==0 && g:filetype_php!=1)
...
function! <SID>GetDoxygenStyleDoc(fun_info, leading_blank)
	...
	"gen param part
	while((g:filetype_php==1 || strlen(type)>0) && strlen(name)>0)
		"if (strlen(type)>0)
			let type = "  : ".type
		"endif
...
function! <SID>GetEmail()
  return "wangruiqiang@renrenshuo.com"
endfunction
...
function! <SID>GetDoxFileHeader(leading_blank)
  ...
  let doc = doc. a:leading_blank."/***************************************************************************\n"
	let doc = doc. a:leading_blank." * \n"
  let doc = doc. a:leading_blank." * Copyright (c) ".<SID>GetYear()." meituan.com, Inc. All Rights Reserved\n"
...
autocmd BufNewFile *.php,*.inc :call <SID>InsertPHPHead()
autocmd BufNewFile *h :call <SID>InsertHeadFormat()

autocmd BufRead,BufNewFile *.php,*.inc :let g:filetype_php=1
